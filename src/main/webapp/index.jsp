<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<!DOCTYPE html>
<html>
<head>
    <title>JSP - Hello World</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet"
          integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
</head>
<body>
<h1>Table of Contents</h1>
<br/>
<%--<h2><a href="${pageContext.request.contextPath}/calculator">Calculator</a></h2>--%>
<h2><a href="calculator">Calculator</a></h2>
<h2><a href="convert-temp">Temperature Converter</a></h2>
<h2><a href="user-json">People Database</a></h2>
<h2><a href="game-json">Game JSON</a></h2>
<h2><a href="julien_measure_converter">Measurement Converter</a></h2>
<h2><a href="test.jsp">Test Page</a></h2>
<h2><a href="users-db">User Db Data</a></h2>
<h2><a href="signup">Sign up</a></h2>
<h2><a href="login">Login</a></h2>
<h2><a href="logout">Logout</a></h2>
<h2><a href="games-db">Game Database</a></h2>
<h2><a href="music-json">Music Json</a></h2>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-C6RzsynM9kWDrMNeT87bh95OGNyZPhcTNXj1NW7RuBCsyN/o0jlpcV8Qyq46cDfL"
        crossorigin="anonymous"></script>
</body>
</html>